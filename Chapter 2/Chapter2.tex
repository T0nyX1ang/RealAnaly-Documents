...
\begin{itemize}[itemindent=2em]
	...
	\begin{center}
	\begin{tikzpicture}
		\draw[red, line width=2pt] (0, 0) circle [radius=2];
		\foreach \x in {1,...,50}
			\draw (0, 0) circle [radius=2 - 1/\x];
		\draw[->, line width=1pt, gray] (0, 0) -- (2, -2);
		\node[below, align=justify] at (0, -2.2) {$F = \bigcup_{k = 1}^{\infty}{F_k}$};
	\end{tikzpicture}
	\qquad
	\begin{tikzpicture}
		\draw[red, line width=2pt] (0, 0) circle [radius=1];
		\foreach \x in {1,...,50}
			\draw (0, 0) circle [radius=1 + 1/\x];
		\draw[->, line width=1pt, gray] (-2, -2) -- (0, 0);
		\node[below, align=justify] at (0, -2.2) {$G = \bigcap_{k = 1}^{\infty}{G_k}$};
	\end{tikzpicture}
	\end{center}
	...
	\item 对集合的适当截取.遇到直接取集合不能满足条件的时候，需要使用相应的开集或闭集截取这个集合，而截取的误差不能太大，应该保持在$\varepsilon$以内.这种方法的原理在于，构造了一个集列以后，利用测度的下连续性，将集列的极限反向转化为集合的可列并，而这个可列并的测度能够等于原来不满足条件的那个集合的测度，这样就能达到效果.如将无限的集合转化为集合与有限集合的交，将无界的集合转化为集合与有界集合的交等.
	\begin{center}
	\begin{tikzpicture}
		\draw (-2, 0) -- (2, 0);
		\draw (-1, -0.1) -- (-1, 0.1);
		\draw (1, -0.1) -- (1, 0.1);
		\draw[->, densely dashed] (0, 0.1) -- (-2, 0.1);
		\draw[->, densely dashed] (0, -0.1) -- (2, -0.1);
		\node at (5, 0) {$A \cap [-n, n] \to A$};
	\end{tikzpicture}
	\end{center}
	...
	\begin{tikzpicture}
		\draw [->] (-2, 0) -- (2, 0);
		\draw [->] (0, -2) -- (0, 2);
		\draw[densely dashed] (0, 0) circle [radius=1.5];
		\draw (0.5, 3.7) .. controls (-2, 0) and (0, -2) .. (3.7, 0.5);
		\node at (5, 0) {$A \cap \overline{U(0, k)} \to A$};
	\end{tikzpicture}
	\end{center}
\end{itemize}
